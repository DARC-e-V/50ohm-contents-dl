<left>
[picture:938:a_photoelement:Schaltzeichen Photoelement (Solarzelle)]
[picture:958:monokristalline Silizium Solarzelle:Aufbau einer monokristallinen Silizium Solarzelle]
</left>
<right>
* Solarzelle wandelt optische Strahlungsenergie in elektrische Energie um
* Grundsätzlicher Aufbau wie in einer Diode
* Durch die Strahlungsenergie werden Elektronen freigesetzt
* Bei Anschluss eines elektrischen Verbrauchers können diese fließen
</right>
---
[question:AB212]
--- style="font-size: 0.7em;"
### Kenngrößen von Solarzellen
| l: Bezeichnung       | c: Abkürzung  | X: Erklärung   |
| Leerlaufspannung | ${U}_{OC}$ |Ist die Spannung, die ohne Last bei voller Sonneneinstrahlung anliegt. |
| Kurzschlussstrom | ${I}_{SC}$  |Ist der Strom, welcher bei einem Kurzschluss am Ausgang, bei voller Sonneneinstrahlung, fließen wird.| 
[table:a_solarzelle:Kenngrößen von Solarzellen]
---
### Zusammenschaltung von Solarzellen

<left>
[picture:949:Solarmodul mit Zellen:Solarzellenverbund in einem Solarmodul]
</left>
<right>
Bei Reihen- und Parallelschaltung verändern sich Klemmenspannung und der maximale Laststrom
</right>
<note>
Analog zur Reihen- und Parallelschaltung von Akkus bzw. Spannungsquellen
</note>

---
[question:AD301]
---
#### Lösungsweg
* gegeben: $U_0 = \qty{0,6}{\volt}$
* gegeben: $I_\text{k} = \qty{1}{\ampere}$
* gegeben: $N_\text{R} = 30, N_\text{P} = 4$
* gesucht: $U_{0,\mathrm{ges}}, I_{k,\mathrm{ges}}$

<fragment>
$U_{0,\mathrm{ges}} = N_\text{R} \cdot U_0 = 30 \cdot \qty{0,6}{\volt} = \qty{18}{\volt}$
</fragment>
<fragment>
$I_{0,\mathrm{ges}} = N_\text{P} \cdot I_\text{k} = 4 \cdot \qty{1}{\ampere} = \qty{4}{\ampere}$
</fragment>
